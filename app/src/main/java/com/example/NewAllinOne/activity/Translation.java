...
public class Translation extends AppCompatActivity {
    ...
    private void descomponerFrase()
    {
        ...
        listaPalabras = Arrays.asList(database.get(objetivo).getDefiniton().split(" "));
    }
    ...
    private void desplegarTranslation()
    {
        tvTranslation.setText(database.get(objetivo).getDefinitionTranslate());
        tvTranslation.setTextSize(16);
    ...
    {
        Controlador.guardar(this, database,5);
        this.finish();
    }
    ...
    private boolean comprobarPosicion(String texto)
    {
        int a =0;
        boolean pasar = false;
        int cantidad =0;
        cantidad = inferior.getChildCount();

        if(texto.equalsIgnoreCase(listaPalabras.get(cantidad)))
        {
            pasar = true;
        }
        return  pasar;
